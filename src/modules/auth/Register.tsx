...
function Register() {
    ...
    const handleOAuth = async (provider: 'google' | 'azure') => {
        ...
        // Nếu không error, redirect sẽ xảy ra tự động
        setLoading(false);
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-md w-full space-y-8">
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    {message && (
                        <div className={`p-3 rounded-md ${message.includes('thành công') || message.includes('tạo') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'
                            }`}>
                            {message}
                        </div>
                    )}
